...
## Reviewer quickstart (recommended path)

If you’re reviewing this repo, this is the fastest “works end-to-end” path:

### 0) What should be running (so you know you’re done)

- **Postgres**: running locally (Pg admin 4) with DB `issue_tracker`
...
#### Option A: Use your local Postgres
...
#### Option B: Run Postgres via Docker (quick + isolated)

```bash
docker run --name issue-tracker-postgres \
...
Then use these server env values:

- `DB_HOST=localhost`
- `DB_PORT=5432`
- `DB_USER=postgres`
- `DB_PASSWORD=your_password_for_pg_admin`
- `DB_NAME=issue_tracker`

### 2) Start the backend + create tables
...
### 4) Review via Swagger (recommended)
...
- Run `POST /auth/login` (or `POST /auth/register`) to get a token
- In Swagger, click **Authorize** and paste the **token value** (Swagger will send it as `Authorization: Bearer <token>`)
- Now you can use “Try it out” on protected endpoints like `/issues`, `/labels`, `/users`, `/issues/{id}/comments`

### 5) Optional: seed demo data

If you want sample issues/labels/users:
...
- Spec generated by `Server/src/config/swagger.ts` via `swagger-jsdoc`

Important reviewer note:

- The Swagger spec currently declares the server as `http://localhost:3000`. For the cleanest review experience, keep `PORT=3000` in `Server/.env`.
